...
int loadPassenger(Passenger list[], int len, TypePassenger listType[], int typeLen, StatusFlight listStatus[], int statusLen)
{
	...
	getValidStingLetrasNombre(name, sizeof(name), "\nIngrese el nombre", "Error, nombre no valido.");
	getValidStingLetrasNombre(lastName, sizeof(lastName), "Ingrese el apellido", "Error, appelido no valido.");
	getValidStingAlfanumerico(flycode, sizeof(flycode), "Ingrese el codigo de vuelo", "Error, codigo no valido.");
